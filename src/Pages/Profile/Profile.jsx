import React, { useState } from 'react'
import Header from '../../Components/Header/Header'
import { FaGraduationCap, FaHandshake } from 'react-icons/fa'
import { MdExplore, MdOutlineKeyboardArrowDown } from 'react-icons/md'
import Achivement from '../../Components/Achivement'

const Profile = () => {
  const tabs = [{ id: 1, name: "Personal data" }, { id: 2, name: "Payment information" }, { id: 3, name: "Notification" }]
  const achievements = [{ id: 1, icon: <MdExplore className='fs-2 text-orange' />, level: 3, bg: "bg-info", title: "Personal data", courses: "total 10 courses", total: 10, achieve: 9 },
  { id: 2, icon: <FaGraduationCap className='fs-2 text-orange' />, level: 2, bg: "bg-success", title: "Payment information", courses: "create 3 courses", total: 3, achieve: 2 },
  { id: 3, icon: <FaHandshake className='fs-2 text-orange' />, level: 1, bg: "bg-warning", title: "Notification", courses: "total 5 courses on one subject", total: 5, achieve: 2 }].map(t => {
    const progress = (t.achieve / t.total) * 100
    return { ...t, progress }
  })

  const [activeTab, setActiveTab] = useState(1)
  const tabHandle = (id) => {
    setActiveTab(id)
  }

  return (
    <>
      <div className="container d-flex justify-content-center align-items-center">
        <div className="row bg-light g-5 p-5 round" style={{ width: "100%", height: "100%" }}>
          <div className="col-12 p-0 m-0">
            <Header />
          </div>
          <div className="col-8 bg-light shadow round">
            <div className='row text-center brd-b fw-bold g-0 d-flex justify-content-around'>
              {tabs.map((t, ind) => (
                <div key={ind} onClick={() => tabHandle(t.id)} className={`col-4 cusrsor-pointer`}>
                  <div className="p-2">
                    {t.name}
                  </div>
                  <div className={`mx-auto ${activeTab === t.id ? 'brd-act-b' : ''}`}></div>
                </div>
              ))}
            </div>

            <div className="p-3">
              <div className="fs-6 fw-bold mb-3">
                Profile photo
              </div>

              <div className="row g-0 align-items-center mb-3">
                <div className="col-2">
                  <img width="100px" height="100px" className='rounded-circle' src="data:image/jpeg;base64," alt="profile" />
                </div>
                <div className="col-10 ps-3">
                  <div className='text-secondary fs-7 mb-2'>JPG, GIF or PNG. Maximum file size 1 MB </div>
                  <button className='btn btn-md btn-secondary rounded ms-2'> Change Photo</button>
                </div>
              </div>

              <form className='row g-3'>
                <div className="col-6 mb-3">
                  <label for="exampleFormControlInput1" className="form-label">UID</label>
                  <input type="email" className="form-control" id="exampleFormControlInput1" placeholder="928-203-1" />
                </div>
                <div className="col-6 mb-3">
                  <label for="exampleFormControlInput1" className="form-label">Nickname</label>
                  <input type="email" className="form-control" id="exampleFormControlInput1" placeholder="@jonsnow" />
                </div>
                <div className="col-6 mb-3">
                  <label for="exampleFormControlInput1" className="form-label">Phone number</label>
                  <input type="email" className="form-control" id="exampleFormControlInput1" placeholder="+91945693653" />
                </div>
                <div className="col-6 mb-3">
                  <label for="exampleFormControlInput1" className="form-label">Email</label>
                  <input type="email" className="form-control" id="exampleFormControlInput1" placeholder="jonsnow@example.com" />
                </div>
                <div className="col-6 mb-3">
                  <label for="exampleFormControlInput1" className="form-label">Name</label>
                  <input type="email" className="form-control" id="exampleFormControlInput1" placeholder="jon@example.com" />
                </div>
                <div className="col-6 mb-3">
                  <label for="exampleFormControlInput1" className="form-label">Surname</label>
                  <input type="email" className="form-control" id="exampleFormControlInput1" placeholder="stark" />
                </div>
                <div className="col-6 mb-3">
                  <label for="exampleFormControlInput1" className="form-label">Middle name</label>
                  <input type="email" className="form-control" id="exampleFormControlInput1" placeholder="-" />
                </div>
                <div className="col-6 mb-3">
                  <label for="exampleFormControlInput1" className="form-label">Date of Birth</label>
                  <input type="email" className="form-control" id="exampleFormControlInput1" placeholder="Dec 12,1997" />
                </div>
              </form>
            </div>
          </div>
          <div className="col-4">
            <div className='round bg-light shadow mb-3'>
              <div className="row g-0 align-items-center">
                <div className="col-2 p-2">
                  <FaGraduationCap className='student_icon fs-3 round text-warning bg-grey' />
                </div>
                <div className="col-9">Student Profile</div>
                <div className="col-1">
                  <MdOutlineKeyboardArrowDown />
                </div>
              </div>
            </div>

            <div className='round bg-light shadow p-2'>
              <div className="fs-4 fw-bold mb-2">Achievements</div>
              {achievements.map((ach, ind) => {
                return (
                  <div className="mb-3">
                    <Achivement key={ind} data={ach} />
                  </div>
                )
              })}
            </div>
          </div>
        </div>
      </div>


    </>
  )
}

export default Profile